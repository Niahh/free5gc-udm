...
// newNrfTestConsumer builds a Consumer over a private UDM context pointed at the
// test NRF and intercepts the openapi cleartext HTTP/2 client with gock.
func newNrfTestConsumer(t *testing.T) *Consumer {
	t.Helper()

	consumer := newTestConsumer(t, &udm_context.UDMContext{
		NfId:   testNfId,
		NrfUri: testNrfUri,
	})
...
func TestSendSearchNFInstances(t *testing.T) {
	...
	gock.New(testNrfUri).
		...
		MatchParam("requester-nf-type", "UDM").
		Reply(http.StatusOK).
	...
	consumer := newTestConsumer(t, &udm_context.UDMContext{NrfUri: testNrfUri})
	targetNFType := models.Nrf_NFMgmt_NFType_UDR
	requesterNFType := models.Nrf_NFMgmt_NFType_UDM
...
func TestSendUpdateNFInstanceWithoutNrfUri(t *testing.T) {
	consumer := newTestConsumer(t, &udm_context.UDMContext{NfId: testNfId})
...
func TestRegisterNFInstance(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// The UDM keeps the instance ID it chose, whatever the NRF echoes back.
